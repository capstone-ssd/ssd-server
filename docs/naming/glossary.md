...
| Paragraph | `Block` 중 텍스트 내용을 담는 하위 개념이다. 이미지까지 포함하는 상위 개념으로 쓰지 않는다. | text block, 문단 | block(전체를 가리킬 때), image | `#139`, `#140` |
| Block | 문서 내부에서 주석, 변경 비교, 순서 부여의 기준이 되는 공통 단위다. 현재 문맥상 text/image를 함께 포괄한다. | document block, 블록 | paragraph(전체를 가리킬 때) | `#138`, `#139`, `#140`, `#141`, `#142` |
...
| Author | `Member`가 문서 작성자 또는 문서 소유자 문맥에서 사용될 때의 역할명이다. 저장 엔티티 이름 자체를 대체하지는 않는다. | owner, 작성자 | member(역할 문맥에서), user | `#139`, `#140` |
| Evaluator | `Member`가 평가자 역할로 사용될 때의 역할명이다. `Reviewer`와 혼용하지 않는다. | reviewer(외부 설명 한정), 평가자 | reviewer(내부 공식 명칭으로 사용) | `#140` |
| Blacklist | 로그아웃되었거나 더 이상 허용하지 않는 access token 식별자를 저장하는 무효화 저장소다. | token blacklist | revoke list, invalid token store | `#144` |
