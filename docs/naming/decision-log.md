...
| 2026-04-14 | `#154` | 문서의 작성/평가 구분은 `DocumentPurpose` enum과 `purpose` 필드로 표현한다. | `evaluation`은 이미 AI 평가 결과 필드로 사용 중이고, `type`/`mode`는 의미가 너무 넓다. 문서의 용도 자체를 드러내는 `purpose`가 가장 명확하다. | Document 생성/조회 API 확장 및 문서 목적 기반 side effect 분기 (`#154`) |
...
| 2026-04-08 | `#137` | `summary`, `shortSummary`, `details`는 이름을 유지하고 의미만 고정한다. | 현재 필드가 이미 저장/응답 모델에 쓰이고 있고, 이 단계에서 rename까지 하면 문서 이슈 범위를 넘는다. 우선 기본 요약, 짧은 요약, 상세 요약의 의미만 고정한다. | Document DTO/응답 의미 정리 (`#140`) |
