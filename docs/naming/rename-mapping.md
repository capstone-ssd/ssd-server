...
| `CreateDocumentParagraphRequest` | `CreateDocumentBlockRequest` | 입력 모델이 텍스트/이미지 block을 함께 다루는 실제 역할을 반영한다. | `#139` | planned |
| `DocumentParagraphDto` | `DocumentBlockResponseItem` | 응답 모델이 block 단위를 표현한다는 점을 드러낸다. | `#139` | planned |
