...
## DTO
- API 요청: `*Request`이며 `ssd-api/.../<context>/request`에 둔다.
- API 응답: `*Response`이며 `ssd-api/.../<context>/response`에 둔다.
- 내부 유스케이스 입력: `*Command`
- 내부 유스케이스 결과: `*Result`
- 검색/필터 입력: 필요 시 `*Criteria`, `*Query`
- 외부 연동 payload: `*Request`, `*Response`, `*DTO`를 사용할 수 있으나 `ssd-domain/.../port/dto` 또는 `ssd-external/.../dto`에 둔다.
