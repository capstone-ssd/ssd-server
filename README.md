...
기획 단계부터 AI 연동 방식, 데이터베이스 벤더, 배포 환경이 여러 차례 변경될 가능성을 고려해 단일 모놀리식 구조에서 멀티모듈 구조로 전환했습니다. 이를 통해 웹, 애플리케이션 트랜잭션, 도메인 로직, 인증, 외부 연동, 영속성 구현의 책임을 분리하고 변경 영향 범위를 줄이는 것을 목표로 합니다.
...
`ssd-domain`은 구현체가 아니라 인터페이스와 도메인 모델을 중심으로 동작합니다. 트랜잭션은 `ssd-application`에서 관리하고, JPA 구현체는 `ssd-infra`에 위치시켜 도메인 로직이 영속성 구현 방식에 직접 의존하지 않도록 구성했습니다.
